...
public class Apples2Apples {
    public static void main(String[] args) {
        ...
        try {
            ...
            // 2. Fill remaining slots with Bots (leave 1 slot for the console player)
            int currentId = players.size();
            while (players.size() < 3) {
                players.add(new BotPlayer(currentId++));
            }

            // 3. Add the local console player (the person hosting the server)
            players.add(new ConsolePlayer(currentId));
